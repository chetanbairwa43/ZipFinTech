                        ...
                        <form action="{{ route('admin.vendor-products.store') }}" method="POST" enctype="multipart/form-data" id="basic-form">
                            ...
                            <div class="row">
                                <div class="form-group col-md-6">
                                    ...
                                    <select name="vendor" class="form-control form-select vendor @error('vendor') is-invalid @enderror" required>
                            ...
                            <div class="variants " id="variants">
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-1">
                                        ...
                                        <input type="number" name="variants[{{$count}}][min_qty]" class="form-control @error('min_qty') is-invalid @enderror min_qty" value="{{ old('min_qty', isset($variant) && isset($variant->min_qty) ? $variant->min_qty : '') }}" required min="1">
                                        @error('min_qty')
                                            <span class="invalid-feedback form-invalid fw-bold" role="alert">
                                                {{ $message }}
                                            </span>
                                    ...
                                    <div class="form-group col-md-1">
                                        ...
                                        <input type="number" name="variants[{{$count}}][max_qty]" class="form-control @error('max_qty') is-invalid @enderror max_qty" value="{{ old('max_qty', isset($variant) && isset($variant->max_qty) ? $variant->max_qty : '') }}" required min="1">
                                        @error('max_qty')
                                            <span class="invalid-feedback form-invalid fw-bold" role="alert">
                                                {{ $message }}
                                            </span>
...
function getProductDetail(product_id, cat_id, vendor_id) {
    var route = "{{ url('admin/vendor-products/products-details') }}/"+product_id+'?&cat_id='+cat_id+'&vendor_id='+vendor_id;
    ...
    $.ajax({
        ...
        success: function(response){
            if(response.output == 'exists') {
                ...
                swal({
                    icon: errorImageURL,
                    title: 'Product Variant already Exists for this Vendor',
                    button: 'Edit',
                })
                .then((willRedirect) => {
                    if (willRedirect) {
                        window.location = "{{ url('admin/vendor-products')}}/"+response.id+'/edit';
                    }
                    return false;
                });
            }
            if(response.success == true) {
                ...
                $('.min_qty').val(response.output.min_qty);
                $('.max_qty').val(response.output.max_qty);
...
$(document).ready(function(){
    ...
    $(document).on('change', '.products', function(){
        ...
        getProductDetail(product_id, cat_id, vendor_id);
    });
    ...
    $('#add_variant_btn').click(function(){
        ...
        data = '<div class="variants " id="variants">'
                + ' <div class="row">'
                    + ' <div class="form-group col-md-2">'
                        + ' <label for="name" class="mt-2"> Quantity <span class="text-danger">*</span></label>'
                        + ' <input type="number" name="variants['+variants_count+'][variant_qty]" class="form-control @error('variant_qty') is-invalid @enderror variant_qty" placeholder="Quantity" value="{{ old('variant_qty', '') }}" required min="0">'
                        + ' @error('variant_qty')'
                            + ' <span class="invalid-feedback form-invalid fw-bold" role="alert">'
                                + ' {{ $message }}'
                            + ' </span>'
                    ...
                    + ' <div class="form-group col-md-3">'
                        ...
                        + ' <select name="variants['+variants_count+'][variant_qty_type]" class="form-control form-select @error('variant_qty_type') is-invalid @enderror variant_qty_type" required>'
                            + ' <option value="" {{ old('variant_qty_type') ? ((old('variant_qty_type') == '') ? 'selected' : '') : '' }} >Select Quantity Type</option>'
                            + ' @foreach($units as $key => $value) '
                                + ' <option value={{$key}} {{ old('variant_qty_type') ? ((old('variant_qty_type') == $key) ? 'selected' : '') : '' }} >{{ $value }}</option>'
                            + ' @endforeach'
                        + ' </select>'
                        + ' @error('variant_qty_type')'
                            + ' <span class="invalid-feedback form-invalid fw-bold" role="alert">'
                    ...
                    + '<div class="form-group col-md-1">'
                        ...
                        + '<input type="number" name="variants['+variants_count+'][min_qty]" class="form-control @error('min_qty') is-invalid @enderror min_qty" value="{{ old('min_qty', '') }}" required min="1">'
                        + '@error('min_qty')'
                            + '<span class="invalid-feedback form-invalid fw-bold" role="alert">'
                                + '{{ $message }}'
                            + '</span>'
                    ...
                    + '<div class="form-group col-md-1">'
                        ...
                        + '<input type="number" name="variants['+variants_count+'][max_qty]" class="form-control @error('max_qty') is-invalid @enderror max_qty" value="{{ old('max_qty', '') }}" required min="1">'
                        + '@error('max_qty')'
                            + '<span class="invalid-feedback form-invalid fw-bold" role="alert">'
                                + '{{ $message }}'
                            + '</span>'
                    ...
                    + ' <div class="form-group col-md-2">'
                        + ' <label for="name" class="mt-2"> Market Price <span class="text-danger">*</span></label>'
                        + ' <input type="number" name="variants['+variants_count+'][market_price]" class="form-control @error('market_price') is-invalid @enderror market_price" placeholder="Market Price" value="{{ old('market_price', '') }}" required min="0" max="9999.99" step="0.01">'
                        + ' @error('market_price')'
                            + ' <span class="invalid-feedback form-invalid fw-bold" role="alert">'
                                + ' {{ $message }}'
                            + ' </span>'
                    ...
                    + ' <div class="form-group col-md-2">'
                        + ' <label for="name" class="mt-2"> Regular Price <span class="text-danger">*</span></label>'
                        + ' <input type="number" name="variants['+variants_count+'][price]" class="form-control @error('price') is-invalid @enderror price" placeholder="Regular Price" value="{{ old('price', '') }}" required min="0" max="9999.99" step="0.01">'
                        + ' @error('price')'
                            + ' <span class="invalid-feedback form-invalid fw-bold" role="alert">'
                                + ' {{ $message }}'
                            + ' </span>'
