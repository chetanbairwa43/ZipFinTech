                    ...
                    <div class="col-lg-10 col-md-10"> 
                        <div class="right-item d-flex justify-content-end" >
                            <form action="" method="GET" class="d-flex">
                                ...
                                <a href="{{ route('admin.orders.index') }}" class="btn-sm reload-btn">
                                ...
                                @if(count($data)>0)
                                    ...
                                    @foreach($data as $key => $value)
                                        <tr data-entry-id="{{ $value->id }}">
                                            ...
                                            <td>{{ isset($value->order_type) ? ($value->order_type == 'O' ? 'Online' : ($value->order_type == 'C' ? 'COD' : '')) : '' }}</td>
                                            <td class="text-center">
                                                <span class="badge badge-opacity-primary text-capitalize">{{ isset($value->status) ? (array_key_exists($value->status,$orderStatus) ? $orderStatus[$value->status] : '') : '' }}</span>
                                            </td>
                                            <td class="text-center">
                                                <a href="{{ route('admin.orders.show', $value->id) }}" class="btn btn-sm btn-icon p-1">
                                                    <i class="mdi mdi-eye mx-1" data-bs-toggle="tooltip" data-bs-offset="0,4" data-bs-placement="top" title="View"></i>
                                                </a>
            
                                                <a href="{{ route('admin.orders.download-invoice', $value->id) }}" class="btn btn-sm btn-icon p-2">
                                                    <i class="mdi mdi-file-pdf" data-bs-toggle="tooltip" data-bs-offset="0,4" data-bs-placement="top" title="Download Invoice"></i>
                                                </a>
                                            </td>
                                        </tr>
                                    @endforeach
